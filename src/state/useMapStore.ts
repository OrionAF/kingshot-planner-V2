...
export const useMapStore = create<MapState & MapActions>()((set, get) => ({
  ...
  createAlliance: (newAllianceData) =>
    set((state) => {
      ...
      const existing = state.alliances.map((a) => parseColorToRgb01(a.color));
      ...
      function passes(c: [number, number, number]) {
        ...
        if (existing.some((e) => dist(e, c) < minColorDistance)) return false;
      ...
      if (!passes(finalRgb)) {
        ...
        while (attempt < maxAttempts) {
          ...
          const lShift = (attempt % 2 === 0 ? -1 : 1) * 0.04 * (attempt + 1);
      ...
      if (rule === 'territory' && def) {
        ...
        if (!ok) {
          ...
          lastResult = {
            ...
            message: 'Must border or be within coverage of alliance territory.',
          };
          return lastResult;
        }
      }
